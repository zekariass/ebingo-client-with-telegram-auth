...
//    const { connected, connecting, reconnectAttempts, connect, reconnect } = useRoomSocket({
//     roomId,
//     enabled: true,
//   })
...
export function ConnectionStatus({ roomId }: { roomId?: number }) {
  const { connected, connecting, reconnectAttempts, reconnect } = useRoomSocket({ roomId, enabled: true })
  ...
  if (connecting) {
    return (
      <div className="flex items-center gap-2 text-sm text-yellow-500">
        <div className="animate-spin rounded-full h-4 w-4 border-2 border-primary border-t-transparent" />
        Connecting{reconnectAttempts ? ` (retry ${reconnectAttempts})` : "..."}
      </div>
    )
  }
