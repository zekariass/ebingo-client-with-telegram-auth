...
//   if (contact.first_name || ctx.from?.first_name) {
//     await ctx.reply(`👋 Hi ${contact.first_name || ctx.from?.first_name}! Registering you now...`);
//   } else {
//     return await ctx.reply('❌ Please set a first name in your Telegram profile and try again.');
//   }

//   ctx.reply('⏳ Registering with telegramId... '+ userId);
...
//           .join('\n');
//         return await ctx.reply(`Registration failed: ${errorMessages}`);
//       }
